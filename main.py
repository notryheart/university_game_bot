...
sheet_2 = client.open_by_key("").sheet1
sheet_3 = client.open_by_key("").sheet1
...
#THANKS CHATGPT FOR THIS DESCRIPTION
characters = {
    "Сергей Королёв": (
        "🚀 <b>Сергей Королёв</b>\n"
...
def choose_team(message):
    ...
    try: #юзаем трай для того, чтобы бот не отлетел к ебеням и не пришлось тратиться на дорогой хостинг с автоподключением
        ...
        bot.send_message(message.chat.id,"Ошибка регистрации, попробуй через 5 секунд")
        print(e)
...
def broadcast_start(message):
    if message.from_user.id != ADMIN_ID:
        ...
        return
    msg = bot.send_message(message.chat.id, "Введи сообщение для рассылки:")
